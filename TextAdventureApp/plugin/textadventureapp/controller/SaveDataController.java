...
public class SaveDataController {
  ...
  public ResponseEntity<?>  save(@RequestBody  SaveRequestDTO request,
    ...
    Set<String> sessionItems = (Set<String>) session.getAttribute(SessionKeys.PLAYER_ITEMS);
    ...
    Boolean foodEventUsed =
        (Boolean) session.getAttribute(SessionKeys.FOOD_EVENT_USED);
  ...
  public ResponseEntity<?> applyLoadedItemsToSession(
    ...
    session.setAttribute(SessionKeys.PLAYER_ITEMS, new HashSet<>(items));

    // シーンもセッションに保持（任意）
    session.setAttribute(SessionKeys.LOADED_SCENE_ID, data.getCurrentSceneId());
    ...
    if (foodEventUsed != null) {
      session.setAttribute(SessionKeys.FOOD_EVENT_USED, foodEventUsed);
    }
